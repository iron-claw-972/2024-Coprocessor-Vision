...
class EdgeTPUModel:
    ...
    def get_normalized_center_coords(self, det, output_image, pad):
        """
        Converts raw prediction bounding box to orginal
        image coordinates.
        ...
        if(len(det)): 
            ...
            ncx = cx/self.get_image_size()[0]
            ncy = cy/self.get_image_size()[1]
    ...
    def get_distance(self, det):
        ...
        if len(det):
            ...
            print(str(estimated_distance)+" inches")
            return estimated_distance
        
    def get_angles(self, det, output_image, pad):
        #source: Limelight docs(LINK HERE)
        if len(det):
           det[:, :4] = self.get_scaled_coords(det[:,:4], output_image, pad)
           horizontal_fov = 0.92 #radians, rough value 
           vertical_fov = 0.92 
           view_plane_width = 2.0*math.tan(horizontal_fov/2)
           view_plane_height = 2.0*math.tan(vertical_fov/2)

           x = view_plane_width/2*self.get_normalized_center_coords(det, output_image, pad)[0]
           y = view_plane_width/2*self.get_normalized_center_coords(det, output_image, pad)[1]

           tx = math.atan2(1,x)*(180/math.pi) #tx in degrees
           ty = math.atan2(1,y)*(180/math.pi) #ty in degrees
